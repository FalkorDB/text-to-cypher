...
use std::fmt::Write;
use crate::chat::{ChatRequest, ChatMessage, ChatRole};
                    ...
                    match forward_to_http_endpoint(tool_args).await {
                        ...
                        Err(e) => {
                            ...
                            Err(CallToolError::new(std::io::Error::other(
                                format!("HTTP forwarding failed: {e}")
                            )))
                        }
                    }
                }
...
async fn forward_to_http_endpoint(tool_args: TextToCypherTool) -> Result<String, Box<dyn std::error::Error + Send + Sync>> {
    ...
    tracing::info!("Forwarding request to HTTP endpoint: {}", serde_json::to_string_pretty(&http_request)?);
